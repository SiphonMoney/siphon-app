...
export default function SimpleSwap({
  ...
  const handleSwap = async () => {
    ...
    console.log('Current state:', { sdkInitialized, swapAmount, swapFromToken });
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Successfully transferred ${swapAmount} ${selectedToken} to ${withdrawAddress}!`);
  ...
  return (
    <div className={`simple-swap ${isLoaded ? 'loaded' : ''}`}>
        ...
        <div className="step-content">
          <ConnectButton 
            ...
            onConnected={async (wallet) => {
              ...
              // Auto-initialize Mock SDK for EVM wallets
              if (wallet.chain === 'EVM' && !sdkInitialized) {
                try {
                  const eth = (window as Window & { ethereum?: unknown })?.ethereum;
                  if (eth) {
                    await initializeWithProvider(eth);
                    setSdkInitialized(true);
                    console.log('Mock SDK auto-initialized for EVM wallet');
      ...
      <div className="swap-inputs">
        <div className="input-group">
          ...
          <TokenSelector 
            ...
            className="token-selector-custom"
          />
        </div>
        ...
        <div className="input-group">
          <input
            type="text"
            placeholder="0.0"
            value={`${swapAmount ? (parseFloat(swapAmount) * 150).toFixed(2) : '0.00'} USDC`}
            readOnly
      ...
      <button 
        ...
        onClick={handleSwap}
        disabled={!sdkInitialized || isTransferring}
      >
        {isTransferring ? (
          ...
          sdkInitialized ? 'Transfer to Address' : 'Connect Wallet First'
        )}
      </button>
    </div>
  );
