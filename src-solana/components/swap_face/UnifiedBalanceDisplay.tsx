...
export default function UnifiedBalanceDisplay({ balances, isSimpleMode = false }: { balances: UnifiedBalance[] | null, isSimpleMode?: boolean }) {
  if (!balances) {
    return (
  ...
  // Simple mode - show only total balances
  if (isSimpleMode) {
    return (
      <div className="balance-display simple-mode">
        <h4>Your Balances</h4>
        {tokenBalances.length === 0 ? (
          ...
          <div className="simple-balances">
            {tokenBalances.map((token, index) => (
              <div key={index} className="simple-balance-item">
                <span className="token-symbol">{token.symbol}</span>
                <span className="token-balance">{parseFloat(token.balance).toFixed(2)}</span>
              </div>
            ))}
          </div>
        )}
    ...
    return (
      <div className="balance-display">
        ...
        <ul style={{ fontSize: '12px', opacity: 0.7, margin: '1rem 0' }}>
          ...
          <li>Mock SDK is still loading balances</li>
