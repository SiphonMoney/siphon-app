...
export async function getMXEPublicKeyWithRetry(
  ...
  if (
    arciumClient?.getMXEPublicKeyWithRetry &&
    typeof arciumClient.getMXEPublicKeyWithRetry === "function"
  ) {
    return (
      arciumClient.getMXEPublicKeyWithRetry as (
        provider: AnchorProvider,
        programId: PublicKey,
        maxRetries: number,
        retryDelayMs: number,
      ) => Promise<Uint8Array>
    )(provider, programId, maxRetries, retryDelayMs);
  }
