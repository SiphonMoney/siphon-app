...
try {
  ...
  console.warn(
    "⚠️  @arcium-hq/client not installed, using placeholder encryption",
  );
  arciumClient = null;
}
...
export const x25519: X25519Interface =
  arciumClient?.x25519 &&
  typeof arciumClient.x25519 === "object" &&
  arciumClient.x25519 !== null
...
export async function encryptOrderData(
  ...
  console.log("Encrypting order (placeholder):", {
    orderData,
    nonce: nonce.toString(),
  });
...
async function decryptWithSignature(
