...
export async function submitEncryptedStrategy(
  ...
  try {
    ...
    // 1. Per-wallet client key (generate + persist on first use).
    const clientKey = await getOrCreateClientKey(userId, cb.onKeygen);

    // 2. Make sure the matching server key is uploaded.
